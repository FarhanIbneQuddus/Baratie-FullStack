...
const Sidebar = ({sidebarOpen, setSidebarOpen}) => {
  ...
  return (
    <div style={{backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), url(${bgall})`, backgroundPosition: 'top left'}} className={`w-60 xl:w-72 bg-gradient-to-r from-yellow-300 to-yellow-200 border-r border-gray-200 flex flex-col justify-between items-center max-sm:absolute top-0 z-20 ${sidebarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
        <div className='w-full'>
          <img onClick={()=> navigate('/')} src={assets.logo} className='w-40 ml-15 my-2 cursor-pointer' alt="" />
        ...
        <div>
          <img src={assets.thousandsunny} className='w-max ml-2 my-2' alt="" />
        </div>
