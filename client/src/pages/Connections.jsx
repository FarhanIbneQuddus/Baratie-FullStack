  ...
  return (
    <div style={{ backgroundImage: `linear-gradient(rgba(0, 0, 0, 0.4), rgba(0, 0, 0, 0.4)), url(${bgall})`, backgroundPosition: '55% 0%' }} className='min-h-screen bg-gradient-to-r from-yellow-100 to-blue-500'>
      <div className='max-w-6xl mx-auto pl-20 pt-20'>
        ...
        {/* Counts */}
        <div className='mb-8 flex flex-wrap gap-6'> {/* Fixed typo: flex-warp to flex-wrap */}
        ...
        <div className='flex flex-wrap gap-6 mt-6'>
          {dataArray.find((item) => item.label === currentTab).value.map((user) => (
            <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-gradient-to-r from-indigo-100 to-blue-300 shadow rounded-md'> {/* Fixed: rounded-mb to rounded-md */}
              <img src={user.profile_picture} alt="" className="rounded-full w-12 h-12 shadow-md mx-auto" />
              <div className='flex-1'>
                <p className='font-medium text-slate-700'>{user.full_name}</p>
                <p className='text-slate-500'>@{user.username}</p>
                <p className='text-sm text-gray-600'>{user.bio ? user.bio.slice(0, 30) + '...' : 'No bio available'}</p> {/* Fixed potential undefined bio */}
